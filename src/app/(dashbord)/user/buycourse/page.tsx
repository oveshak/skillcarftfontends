...
type CourseCard = {
  id: number;             // course id
  enrollId?: number;      // <-- NEW: enrolled id to pass forward
  title: string;
  slug?: string;
  thumb?: string;
  price?: number;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // course_id কে unique ধরে একই course একবারই দেখাচ্ছি
        const map = new Map<number, CourseCard>();
        for (const r of rows) {
          const cd = r?.course_details;
          if (!cd?.id) continue;
          ...
          if (!map.has(cd.id)) {
            map.set(cd.id, {
              id: cd.id,
              enrollId: r?.id, // <-- keep the enrolled id
              title: cd.title || 'Unnamed Course',
              slug: cd.slug,
              thumb: cd.course_thumbnail,
              price: cd.price,
        ...
        <div className="max-w-7xl mx-auto">
          <div className="px-4 sm:px-6 pb-6">
            ...
            {!loading && !error && (
              <div className="space-y-3 mb-4">
                {visibleCourses.map((c) => {
                  // === DESIGN অপরিবর্তিত, শুধু href-এ enroll_id query param add হলো ===
                  const query = c.enrollId ? `?id=${encodeURIComponent(String(c.enrollId))}` : '';
                  const href = c.slug
                    ? `/user/ourcourse/${c.slug}${query}`
                    : `/user/ourcourse/id/${c.id}${query}`;
